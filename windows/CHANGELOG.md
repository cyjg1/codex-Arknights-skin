...
- 新增右上角 `NATIVE UI` 一键切换，可立即移除主题背景、样式、侧栏装饰与控制器，回到 Codex 原生界面。
- 原生模式仅保留一个可访问的 44×44 `△ RI` 恢复按钮，可一键切回完整主题，并自动适配原生浅色/深色背景。
- 主题启用状态保存在本机；关闭期间自动轮播暂停，恢复时保留原有干员、轮播与显示模式偏好。
- 验证器与渲染回归测试现同时覆盖主题模式、原生模式及双向往返切换。
...
- 移除任务正文和用户消息的卡片底色，让角色原画透过文字区，并用整幅渐变安全区代替描边与多层文字光晕。
- 移除原生任务标题栏的白色/磨砂底条，标题直接显示在主题背景上。
- 为左侧栏增加罗德岛三角徽记水印、舰船编号刻度和状态灯，并保持滚动、选择与鼠标事件完全可用。
- README 改用三张仅含演示数据的真实主题渲染截图，并清理旧图库、推广素材与未使用参考图。
