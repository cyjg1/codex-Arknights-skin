...
- 新增阿米娅、凯尔希、陈、德克萨斯、能天使与 W 六套角色主题。
- 新增手动选角、12 秒自动轮播和可持久化的轮播开关。
- 新增跟随 Codex、强制白天、强制夜间三档显示，并为两套界面分别校准对比度。
- 将自动模式改为按本地时间切换白天/夜间，并增加宽屏角色留白布局。
- 将底部干员条改为默认折叠的右上角控制器，避免遮挡输入框与模型控件。
- 兼容新版 Codex 的嵌套 `desktop.appearanceLightChromeTheme` 配置表。
- 修复同版本热更新复用旧样式和旧控制器事件的问题。
- 修复当前 .NET 运行时下原子替换要求非空备份路径的问题。
